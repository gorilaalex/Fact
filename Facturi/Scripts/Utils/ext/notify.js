...
(function($){
    ...
    var ele, space, timer,
            ...
            events = {
                close: function () {
                    ...
                    $('.notify-close').bind('keydown', function () {
                        hide(ele);
                        hide(space);
                        $(this).unbind('keydown');
                    });
            ...
            handle = function (type, txt, options) {
                ...
                if (typeof (options) === 'object') {
                    if (options.close) {
                        txt = txt + ' <div class="notify-close" tabindex="999"></div>';
                        show(ele, type, txt);
                        events.close(ele)
                    } else {
                        show(ele, type, txt);
    ...
    $.extend({
        notify : {
            ...
            basicWithButton: function (txt, displayAs, options, callback) {
                txt = txt + ' <input type="button" class="blue-box-button" value="' + displayAs + '" id="btn-extendSession" />'
                handle('basic', txt, options);
                $("#btn-extendSession").click(function () {
